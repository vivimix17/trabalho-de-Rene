...
def menu_admin():
    ...
    while usuario_logado:
        ...
        print("6. Ordenar por ordem decrescente Notícia")
        print("7. Exibir em ordem decrescente por curtidas as notícias publicadas pelo editor")
        print("8. Exibir em ordem decrescente por curtidas todas as notícias")
        print("9. Salvar arquivo contendo todas as notícias do editor")
        print("10. Logout")
        ...
        if choice == '1':
            ...
            exibir_noticias_editor_ordem_decrescente()
        elif choice == '8':
            exibir_noticias_todos_editores_ordem_decrescente()
        elif choice == '9':
            salvar_noticias_editor_em_arquivo()
        elif choice == '10':
            usuario_logado = None

def salvar_noticias_editor_em_arquivo():
    nome_arquivo = f"{usuario_logado}_noticias.txt"
    noticias_editor = [noticia for noticia in noticias if noticia['editor'] == usuario_logado]
    with open(nome_arquivo, 'w', encoding='utf-8') as arquivo:
        for noticia in noticias_editor:
            arquivo.write(f"Título: {noticia['titulo']}\n")
            arquivo.write(f"Conteúdo: {noticia['conteudo']}\n")
            arquivo.write(f"Autor: {noticia['autor']}\n")
            arquivo.write(f"Curtidas: {noticia['curtidas']}\n")
            arquivo.write("Comentários:\n")
            for comentario in noticia['comentarios']:
                arquivo.write(f"   {comentario}\n")
            arquivo.write("\n")
def exibir_noticias_editor_ordem_decrescente():
    print("\nNotícias do Editor (Ordem Decrescente por Curtidas):")
    noticias_editor = [noticia for noticia in noticias if noticia.get('editor') == usuario_logado]
...
def exibir_noticias_todos_editores_ordem_decrescente():
    print("\nNotícias de Todos os Editores (Ordem Decrescente por Curtidas):")
    todas_noticias = noticias.copy()
    todas_noticias.sort(key=lambda x: x['curtidas'], reverse=True)
...
def exibir_noticias_adm():
    print("\nNotícias do Editor (Ordem Decrescente por Curtidas):")
    noticias_editor = [noticia for noticia in noticias if noticia['autor'] == usuario_logado]
    noticias_editor.sort(key=lambda x: x['curtidas'], reverse=True)
    for i, noticia in enumerate(noticias_editor):
        print(f"{i + 1}. Título: {noticia['titulo']}")
        print(f"   Autor: {noticia['autor']}")
